...
client.on("ready", () => {
    ...
    console.log(`${colors.cyan}[ MUSIC ]${colors.reset} ${colors.green}Akxsh Music System Ready 🎵${colors.reset}`);
...
app.listen(port, () => {
    ...
    console.log(`${colors.cyan}[ TIME ]${colors.reset} ${colors.gray}${new Date().toISOString().replace('T', ' ').split('.')[0]}${colors.reset}`);
    console.log(`${colors.cyan}[ USER ]${colors.reset} ${colors.yellow}akxsh9${colors.reset}`);
});
